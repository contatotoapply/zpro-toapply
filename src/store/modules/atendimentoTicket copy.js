...
const checkTicketFilter = (ticket) => {
  ...
  console.log('isQueuesTenantExists', isQueuesTenantExists)
  if (isQueuesTenantExists) {
    const isQueueUser = UserQueues.findIndex(q => ticket.queueId === q.id)
    console.log('isQueueUser',isQueueUser)
    if (isQueueUser !== -1) {
      console.log('Fila do ticket liberada para o Usuario A', ticket.queueId)
      console.log('Fila do ticket liberada para o Usuario B', profile, ticket.queueId)
      if (profile === 'admin' && filasSelecionadas.length > 0){  
        if (filasSelecionadas.includes(ticket.queueId)) return true
        return false 
        // validFilter = true
      }
      ...
      if (profile === 'admin' && ticket.queueId) return true
      return false
    }
  }
