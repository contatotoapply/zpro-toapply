      ...
      socket.on(`${usuario.tenantId}:whatsappSession`, data => {
        ...
        if (data.action === 'readySession') {
          this.$q.notify({
            position: 'top',
            icon: 'mdi-wifi-arrow-up-down',
            message: `A conexão foi atualizada. Conexão: ${data.session.name}. Número: ${data.session.number}.`,
            type: 'positive',
            color: 'primary',
            html: true,
            progress: true,
      ...
      socket.on(`${usuario.tenantId}:wavoip`, data => {
        ...
        if (data.action === 'update') {
          this.$q.notify({
            position: 'bottom',
            icon: 'mdi-wifi-arrow-up-down',
            message: `A conexão com o WhatsApp está sendo sincronizada com o Wavoip. Conexão: ${data.session.name} | Wavoip Token: ${data.session.wavoipToken}.`,
            type: 'positive',
            color: 'positive',
            html: true,
            progress: true,
        ...
        if (data.action === 'update') {
          if (data.status.all === -1 && data.status.this === -1) {
            ...
            notification = this.$q.notify({
              message: `Aguarde, iniciando o processo de sincronização com o App.`,
              type: 'warning',
              progress: true,
              position: 'top',
              actions: [{
            ...
            notification = this.$q.notify({
              message: `Sincronizando mensagens ${data.status.this} de ${data.status.all} em ${data.status.date}`,
              type: 'warning',
              progress: true,
              position: 'top',
              actions: [{
