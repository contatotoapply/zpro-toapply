...
<template>
  <div>
    ...
    <q-dialog v-model="confirmDelete" persistent>
      <q-card style="min-width: 350px">
        <q-card-section>
          <div class="text-h6">Você tem certeza que deseja excluir esse fluxo?</div>
          <div>{{ chatFlowSelecionado.name }}</div>
        </q-card-section>
        <q-card-actions align="right" class="text-primary">
          <q-btn
    ...
    <q-dialog v-model="modalImportarJson" persistent>
      <q-card style="min-width: 350px">
        <q-card-section>
          <div class="text-h6">Importar JSON para {{ chatFlowSelecionado.name }}</div>
          <q-input
            v-model="jsonContent"
            type="textarea"
            label="JSON"
            autogrow
            filled
            :rules="[val => !!val || 'JSON é obrigatório']"
          />
        </q-card-section>
    ...
    async importarJsonConfirm() {
      try {
        ...
        if (!isValid) {
          this.$q.notify({
            type: 'negative',
            message: 'JSON inválido. Verifique o formato e as propriedades obrigatórias.'
          });
          return;
        }
