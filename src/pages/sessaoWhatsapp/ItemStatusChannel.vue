    ...
    <q-item>
      ...
      <q-item-section>
        ...
        <q-item-label v-if="item.status == 'DISCONNECTED'">
          ...
          <span class="row col"
            v-if="item.type === 'whatsapp' || item.type === 'baileys'"> Certifique-se de que seu celular esteja conectado à internet e tente
        ...
        <q-item-label v-if="item.status == 'CONNECTED' && item.type === 'meow'">  
          <span class="text-weight-medium blur-effect"> Conexão estabelecida: {{item.wabaId}}</span>
        </q-item-label>
        ...
        <q-item-label v-if="item.status == 'CONNECTED' && item.type === 'whatsapp' || item.type === 'baileys' || item.type === 'waba' || item.type.includes('hub')">
          <template v-if="item.profilePic !== 'disabled'">
