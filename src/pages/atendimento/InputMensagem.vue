      ...
      <q-dialog v-model="modalGravacaoVideoVisivel" @hide="fecharModalGravacaoVideo">
        <q-card class="q-pa-sm" style="max-width: 100%; width: 500px;">
          ...
          <q-card-actions align="center" class="q-mb-sm">
            <q-btn v-if="!isRecordingVideo" @click="handleStartRecordingVideo" label="Iniciar Gravação" color="primary" class="full-width q-mb-xs" />
            <q-btn v-if="isRecordingVideo" @click="handleStopRecordingVideo" label="Enviar Gravação" color="primary" class="full-width q-mb-xs" />
            <q-btn @click="fecharModalGravacaoVideo" label="Cancelar" color="negative" class="full-width" />
          </q-card-actions>
        </q-card>
      </q-dialog>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      gptEnabled: false,
      loading: false,
      accept: '.csv, .rar, .kml, .psd, .txt, .xml, .jpg, .png, .pdf, .doc, .docx, .mp4, .xls, .xlsx, .jpeg, .zip, .ppt, .ogg, .mp3, .pptx, image/*, .mpeg, .pfx, .p2k',
  ...
  methods: {
    ...
    handleCancelRecordingVideo() {
        if (this.mediaRecorder) {
            this.cancelRecording = true; // Sinaliza que a gravação deve ser cancelada
            this.mediaRecorder.stop();
            this.isRecordingVideo = false;
        }
    },
    ...
    async listParticipantes() {
      if (this.ticketFocado.status === 'undefined' || this.ticketFocado.status === 'closed') return;
    ...
    async handleStopRecordingAudio() {
      ...
      try {
        ...
        } else if (this.semRedis === 'disabled' && !this.ticketFocado.channel.includes('hub')){
          await EnviarMensagemTexto(ticketId, formData)
        }
        //}
        ...
        //   if (this.semRedis === 'enabled'){
        //     console.log('1')
        //     if(!message.scheduleDate && ['whatsapp', 'baileys'].includes(this.ticketFocado.channel)){
        //       formData.append('medias', blob, filename)
        //       formData.append('body', filename)
        //       formData.append('fromMe', true)
        //       formData.append('whatsappId', this.ticketFocado.whatsappId);
        ...
        //       formData.append('ticket', JSON.stringify(this.ticketFocado)); 
        //       await NoRedis(formData)
        //     } else if(message.scheduleDate || ['whatsapp', 'baileys'].includes(this.ticketFocado.channel)){
        //       await EnviarMensagemTexto(ticketId, message)
        //     }
    ...
    onRejectedFiles(rejectedEntries) {
      this.$q.notify({
        ...
        <ul>
          ...
          <li>Extensões aceitas: txt, .jpg, .png, image/jpeg, .jpeg, image/*, .pdf, .doc, .docx, .xls, .xlsx, .zip, .ppt, .pptx, .mp4, .mp3, .ogg</li>
        </ul>`,
        type: 'negative',
        progress: true,
        position: 'top',
    ...
    async listTenantPorId(){
      ...
      console.log('socket ON: FAST MESSAGING:', this.semRedis)
    },
  },
  beforeMount() {
