      ...
      <div v-for="(mensagem, index) in mensagens" :key="`mensagem-${index}`">
        ...
        >
          ...
          <div style="min-width: 100px; max-width: 350px" :style="mensagem.isDeleted ? 'color: rgba(0, 0, 0, 0.36) !important;' : ''">
            ...
            <q-btn v-if="!mensagem.isDeleted && isShowOptions" class="absolute-top-right" dense flat ripple round icon="mdi-chevron-down">  
              <q-menu square auto-close anchor="bottom left" self="top left">
                <q-list style="min-width: 100px">
                  ...
                  <q-item :disable="!['whatsapp', 'telegram', 'baileys', 'waba'].includes(ticketFocado.channel)" clickable @click="mensagemReacao = mensagem; modalEmojiOpen = true" v-if="(mensagem.mediaType !== 'notes' && mensagem.mediaType !== 'transcription') && (ticketFocado.channel === 'whatsapp' || ticketFocado.channel === 'baileys' || ticketFocado.channel === 'waba' )">
                    <q-item-section>Reagir</q-item-section>
                  </q-item>
                  <q-item :disable="!['whatsapp', 'telegram', 'baileys'].includes(ticketFocado.channel)" clickable @click="abrirModalEdicao(mensagem)" v-if="(mensagem.mediaType === 'chat' || mensagem.mediaType === 'extendedTextMessage' || mensagem.mediaType === 'conversartion' ) && (ticketFocado.channel === 'whatsapp' || ticketFocado.channel === 'baileys') && !mensagem.scheduleDate">
                  ...
                  <q-separator />
                  <q-item @click="deletarMensagem(mensagem)" clickable v-if="mensagem.fromMe && (mensagem.mediaType !== 'notes' && mensagem.mediaType !== 'transcription')" :disable="isDesactivatDelete(mensagem) || (ticketFocado.channel === 'messenger' || ticketFocado.channel === 'waba')">
                    <q-item-section>
            ...
            <template style="margin-top:20px" v-if="mensagem.mediaUrl && !['media', 'audio', 'vcard', 'contactMessage','locationMessage', 'image', 'imageMessage', 'video', 'videoMessage', 'sticker', 'location', 'interactive', 'button', 'list', 'button_reply', 'sticker', 'notes', 'transcription'].includes(mensagem.mediaType)">
