import { AtualizarStatusTicketForcado, AtualizarStatusTicket, EnviarMensagemTexto, LocalizarMensagens, AtualizarTicket, ConsultarDadosTicket} from 'src/service/tickets'
  ...
  methods: {
    ...
    async atualizarStatusTicket (status) {
      ...
      this.$q.dialog(dialogOptions).onOk(async (data) => {
        ...
        if(this.ticketFocado.whatsapp.sendEvaluation === 'enabled' && this.ticketFocado.status === "open" && (this.ticketFocado.channel === 'whatsapp' || this.ticketFocado.channel === 'waba' || this.ticketFocado.channel === 'baileys' || this.ticketFocado.channel.includes("hub_")) && !this.ticketFocado.isGroup){
          try{
    ...
    async fecharTicket (ticket) {
      ...
      this.$q.dialog(dialogOptions).onOk(async (data) => {
        AtualizarStatusTicketForcado(ticket.id, 'closed')
          .then(res => {
            this.loading = false
            this.$q.notify({
              message: `'Atendimento encerrado!' || (Ticket ${ticket.id})`,
              type: 'positive',
              progress: true,
              actions: [{
                icon: 'close',
